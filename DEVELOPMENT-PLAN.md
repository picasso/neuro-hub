# Детальный план разработки NeuroGig MVP

Этот документ содержит пошаговый план разработки платформы NeuroGig согласно функциональной спецификации из MVP.md и архитектурным решениям из ARCHITECTURE-DECISIONS.md.
...
- [x] `GET /api/freelancers` - каталог фрилансеров с фильтрами
- [ ] `POST /api/invitations` - приглашение фрилансера
- [ ] `GET /api/projects/:id/applications` - просмотр заявок на проект
- [ ] `PUT /api/applications/:id/status` - принятие/отклонение заявки
- [ ] `POST /api/favorite-freelancers` - добавление фрилансеров в избранное
- [ ] `GET /api/favorite-freelancers` - список избранных фрилансеров
...
- [ ] Шаг 1: Название и категория и возможно баннер проекта
...
- [x] Страница каталога фрилансеров
- [x] Карточка фрилансера в каталоге
- [ ] Фильтры (категория, навыки, рейтинг, цена, доступность)
- [ ] Сортировка (рейтинг, цена, кол-во проектов)
- [x] Детальная страница профиля фрилансера (view only)
- [ ] Кнопка приглашения в проект
- [ ] Список избранных фрилансеров

### 5.5 UI управления проектами
- [ ] Страница списка проектов заказчика
- [ ] Страница заявок на конкретный проект
- [ ] Сортировка заявок (релевантность, цена, рейтинг, дата)
- [ ] Карточка заявки с информацией о фрилансере
...
- [x] Выбрать подход для real-time (Server-Sent Events vs WebSockets)
- [x] Реализовать real-time для мессенджера
...
- [x] Тесты для бизнес-логики
- [x] Тесты для валидационных схем Zod
- [x] Тесты для утилитных функций
