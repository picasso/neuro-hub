...
export function ChatConversationsList({
	...
	return (
		<div className="flex h-full min-h-96 flex-col rounded-2xl border border-border bg-background">
			<Stack justify="space-between" className="border-b border-border px-4 py-4">
				<Stack vertical gap={0.5} align="start">
					<TS clean variant="subtitle" content="Диалоги" />
					<TS
						variant="caption"
						color="secondary"
						content="Только существующие project-scoped диалоги"
					/>
				</Stack>
				<Button
					type="button"
			...
			{conversationsError && conversations.length === 0 ? (
				...
				<div className="flex flex-1 items-center justify-center p-4">
					<Empty
						...
						helper="В этой фазе workspace показывает только уже существующие чаты."
					/>
				</div>
			) : (
											...
											>
												...
												>
													<TS
														...
														content={conversation.otherParticipant.name}
													/>
													<TS
														variant="caption"
														color="secondary"
														className="truncate"
														content={formatParticipantRole(
												...
												<TS
													...
													content={formatConversationTime(
...
function formatParticipantRole(role: ChatConversationSummary['otherParticipant']['role']) {
